...
| 2026-08-07 | Decided the GOMAC hub's implementation language/runtime: **TypeScript/Node**, closing that open question. Driven by the daemon's I/O-orchestration shape (not compute-bound, so C/C++'s advantages don't pay off here even though it's a legitimate strength — with a real home instead in the ESP32 firmware layer), by the compute Pi being confirmed **headless** (any GUI is necessarily served over the network to a separate device, making real-time WebSocket push a core requirement — Node's strength), and by direct code/pattern reuse from BojuBot. That reuse is now a real plan, not just a source of inspiration: extract genuinely reusable pieces out of BojuBot into a shared package (Claude Code CLI wrapping, permission modes, chat-UI components, WebSocket state sync) rather than hand-copying patterns — plausibly useful to others building similar tools, given both projects are MIT. Also captured an open brainstorm on voice input: wake-word and push-to-talk as complementary (not competing) mechanisms, PTT considered as either a phone app or a wired CB-radio-style handset (or both) — mechanism undecided, but the routing principle is settled: PTT is a generic trigger-plus-audio-stream concept that flows through MQTT like everything else in this project, with a wired handset connecting to the nearest ESP32 node rather than running a direct wire the length of the van to the headless compute Pi. |
| 2026-08-07 | Answered a real design question: **can GOMAC write new HA automations from a plain-English description, not just trigger existing ones? Yes.** Found real prior art confirming the pattern works — [AItomation](https://github.com/gmatrangola/AItomation), an existing open-source project doing exactly this (plain English → LLM-generated automation → pushed live via HA's REST API) — added as a second architectural reference alongside BojuBot. Also found HA ships its own built-in LLM API for exposing entities/services to an AI; flagged as an open design fork whether GOMAC leans on that vs. its own custom toolset against HA's config API, or both. Designed the permission-model tie-in: automation authoring is "full" tier, gated as **propose-then-confirm** (draft shown for approval before going live) rather than silent-write-and-enable — the concrete second example the Permission Model section needed beyond the vague CAN-writes placeholder. Also noted GOMAC's automation authoring should ask clarifying questions on ambiguous requests rather than guess at trigger semantics, mirroring the same discipline already governing how Claude Code works in this repo. |
